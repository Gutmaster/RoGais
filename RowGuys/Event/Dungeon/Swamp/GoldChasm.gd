...
func _on_Ignore_pressed():
	...
	$EventResult/CenterContainer/EventResult/Text.text = "There's no telling what's in that water. Time to leave."
...
func MemberRetrieve(member):
	...
	if(member.hp > 0):
		...
		$EventResult.replace_by_instance()
		$EventResult/CenterContainer/EventResult/Text.text = member.get_name() + " Dives deep into the pool and gathers as much treasure as they can carry. Suddenly, they are swarmed by hungry pirahnas. They drop the treasure but are still torn to shreds before they can surface."